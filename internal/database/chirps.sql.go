...
func (q *Queries) GetByID(ctx context.Context, id uuid.UUID) (Chirp, error) {
...
const getByID2 = `-- name: GetByID2 :many
SELECT id, created_at, updated_at, body, user_id
FROM chirps
WHERE user_id = $1
`

func (q *Queries) GetByID2(ctx context.Context, userID uuid.UUID) ([]Chirp, error) {
	rows, err := q.db.QueryContext(ctx, getByID2, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
